# -*- coding: utf-8 -*-
"""
Created on Wed Sep 19 19:03:06 2018
...
def computeCost(X, y, theta):
  ...
  cost=((2*yhat.shape[0])**-1)*np.sum((((yhat)-y))**2)
  return cost
...
def computeGradient(X,y,theta):
  ...
  n=len(theta)
  grad = np.zeros(n)
  #partial diff of J(theta)= (2*(alpha/yhat.shape[0]))*np.sum((((yhat)-y))**2)
  yhat=predict(X,theta)
  grad= (np.dot((yhat-y),X))/yhat.shape[0]
  return grad
...
def main():
  ...
  data1 = np.loadtxt("housing.csv", delimiter=",");
  X1 = data1[:, [0, 1]]
  y1 = data1[:, 2]
