# -*- coding: utf-8 -*-
"""
Created on Wed Oct 10 13:10:36 2018
...
def computeCost(X, y, theta, lambd):
  ...
  ##### replace the next line with your code #####
  m=len(X)
  h_thetaX=np.dot(X,theta)
  z=(1-np.multiply(y,h_thetaX))
  J_theta=(m**-1)*(np.sum(np.maximum(z,0))+(lambd*np.dot(theta,theta)))
  cost = J_theta
...
def computeGradient(X,y,theta,lambd):
  ...
  grad = np.zeros(n)
  m=len(X)
  z = np.where(np.multiply(y, np.dot(X, theta)) <= 1, 1, 0)
  for index1 in range(len(theta)):
    grad[index1]= (2*lambd*theta[index1])-((m**-1) * np.dot(np.multiply(y,X[:,index1]),z))
    #grad[index1] = 2*lambd*theta-((m**-1) * np.sum(np.multiply(np.multiply(y,X[:,index1]),z)))
  return grad
